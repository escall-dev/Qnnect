...
function hasTeacherScheduleConflict($conn, $teacher_username, $school_id, $day_of_week, $start_time, $end_time, $exclude_id = null) {
    ...
    if ($exclude_id !== null) {
        // 10 parameters: teacher_username(s), school_id(i), day_of_week(s), 
        // end_time(s), start_time(s), end_time(s), start_time(s), start_time(s), end_time(s), exclude_id(i)
        $stmt->bind_param("sisssssssi", $teacher_username, $school_id, $day_of_week, 
                         $end_time, $start_time, $end_time, $start_time, 
                         $start_time, $end_time, $exclude_id);
    } else {
        // 9 parameters: teacher_username(s), school_id(i), day_of_week(s), 
        // end_time(s), start_time(s), end_time(s), start_time(s), start_time(s), end_time(s)
        $stmt->bind_param("sisssssss", $teacher_username, $school_id, $day_of_week, 
                         $end_time, $start_time, $end_time, $start_time, 
                         $start_time, $end_time);
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($is_update) {
        ...
        if (hasTeacherScheduleConflict($conn, $teacher_username, $school_id, $day_of_week, $start_time, $end_time, $schedule_id)) {
        ...
        if (hasTeacherScheduleConflict($conn, $teacher_username, $school_id, $day_of_week, $start_time, $end_time)) {
            echo json_encode(['success' => false, 'message' => 'Schedule conflicts with an existing schedule']);
            exit;
        }
        ...
        $sql = "INSERT INTO teacher_schedules 
                (teacher_username, subject, section, day_of_week, start_time, end_time, room, school_id) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssssssi", $teacher_username, $subject, $section, $day_of_week, 
                          $start_time, $end_time, $room, $school_id);
