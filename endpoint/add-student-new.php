...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['student_name'], $_POST['generated_code'], $_POST['face_verified'], $_POST['face_image_data'])) {
        ...
        try {
            ...
            header("Location: http://localhost/personal-proj/Qnnect/masterlist.php?add_success=1&student_id=$inserted_id");
            exit();
            
        } catch (Exception $e) {
            error_log("Error saving student data: " . $e->getMessage());
            // Redirect with error
            header("Location: http://localhost/personal-proj/Qnnect/masterlist.php?add_error=1&message=" . urlencode("Error saving student: " . $e->getMessage()));
            exit();
        }
    ...
    // Redirect with error
    header("Location: http://localhost/personal-proj/Qnnect/masterlist.php?add_error=1&message=" . urlencode("Invalid request method!"));
    exit();
}
